...
export const DownloadButton = ({
  ...
  const { isDownloaded, isDownloading, getProgress, download, deleteDownload } = useDownload();
  ...
  const buttonClass = size === "sm" ? "p-1.5 rounded-full" : "p-2 rounded-lg";
  ...
  if (progress?.status === "error") {
    return (
      ...
      >
        ...
        {showLabel && <span className="ml-2 text-sm">{t("downloads.error")}</span>}
      </button>
    );
  }
  ...
  if (downloading) {
    return (
      ...
      >
        ...
        {showLabel && <span className="ml-2 text-sm">{t("downloads.downloading")}</span>}
      </button>
    );
  }
  ...
  if (downloaded) {
    return (
      ...
      >
        ...
        {showLabel && (
          <span className="ml-2 text-sm group-hover:hidden">{t("downloads.downloaded")}</span>
        )}
      </button>
    );
  }
  ...
  return (
    ...
    >
      ...
      {showLabel && <span className="ml-2 text-sm">{t("downloads.download")}</span>}
    </button>
  );
};
